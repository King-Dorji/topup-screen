  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[201] flex items-center justify-center p-4 pointer-events-none">
        ...
        {hasEnoughBalance ? (
          /* ── Normal OTP flow ── */
          <div className="modal-enter pointer-events-auto w-[min(460px,100%)] h-[min(600px,calc(100vh-80px))] rounded-2xl overflow-hidden flex flex-col bg-white shadow-[0_32px_80px_rgba(0,0,0,0.55),0_0_0_1px_rgba(255,255,255,0.06)]">
            <button onClick={onClose} className="absolute top-3 right-3 z-10 w-7 h-7 rounded-full border-none bg-[rgba(0,0,0,0.08)] cursor-pointer text-[13px] text-grey-700">✕</button>
            <iframe ref={iframeRef} src={`/cashia-pay?amount=${amount}`} className="w-full h-full border-none block" title="Cashia Payment" sandbox="allow-scripts allow-same-origin allow-forms" />
          </div>
        ) : (
...
function InsufficientBalanceCard({ amount, cashiaBalance, onClose }: { amount: number; cashiaBalance: number; onClose: () => void }) {
  ...
  return (
    <div className="modal-enter pointer-events-auto w-[min(420px,100%)] rounded-[20px] bg-white shadow-[0_32px_80px_rgba(0,0,0,0.55),0_0_0_1px_rgba(255,255,255,0.06)] overflow-hidden">
      ...
      <div className="px-6 py-5">
        ...
        <div className="bg-grey-50 rounded-xl p-4 mb-4 border border-grey-150">
          <div className="flex justify-between mb-2">
            ...
            <span className="text-[13px] font-bold text-grey-900">KES {amount.toLocaleString()}</span>
          </div>
          <div className="flex justify-between mb-2">
            <span className="text-[12px] text-grey-500">Your Cashia balance</span>
            <span className="text-[13px] font-bold text-system-green">KES {cashiaBalance.toLocaleString()}</span>
          </div>
        ...
        {phase === "info" && (
          <>
            <p className="m-0 mb-4 text-[13px] text-grey-600 leading-relaxed text-center">
              Top up your Cashia wallet with at least <strong className="text-cashia-pink-500">KES {needed.toLocaleString()}</strong> to complete this deposit.
            </p>
            <button
            ...
            <button onClick={onClose} className="w-full py-[11px] rounded-[10px] border border-grey-150 bg-transparent text-grey-500 text-[13px] font-semibold cursor-pointer">
              Cancel
            </button>
          </>
        )}
        ...
        {(phase === "counting" || phase === "redirecting") && (
          <div className="text-center py-2 pb-1">
            ...
            {/* Countdown circle — inline styles required for dynamic strokeDashoffset */}
